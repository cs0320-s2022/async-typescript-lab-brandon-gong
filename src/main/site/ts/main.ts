...
const sunDropdown = document.getElementById("sun") as HTMLSelectElement;
const moonDropdown = document.getElementById("moon") as HTMLSelectElement;
const risingDropdown = document.getElementById("rising") as HTMLSelectElement;

// Here, when the value of sun is changed, we will call the method postAndUpdate.
// TODO: Do the same for moon and rising
sunDropdown.addEventListener("change", postAndUpdate);
moonDropdown.addEventListener("change", postAndUpdate);
risingDropdown.addEventListener("change", postAndUpdate);
...
function postAndUpdate(): void {
  ...
  const postParameters: {sun: string, moon: string, rising: string} = {
  ...
  fetch("http://0.0.0.0:4567/matches", {
    method: 'POST',
    headers: {
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.forEach(m => suggestionList.innerHTML += `<li tabindex="0">${m}</li>`);
}
...
async function reset(event: KeyboardEvent) {
  if (event.key === "a") {
    updateValues("Pisces", "Pisces", "Pisces");
    postAndUpdate();
  }
}
